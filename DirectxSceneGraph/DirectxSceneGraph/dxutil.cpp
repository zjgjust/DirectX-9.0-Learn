...
HRESULT DXUtil_GetDXSDKMediaPathCch( TCHAR* strDest, int cchDest )
{
    ...
    if( lstrlen(strDest) + lstrlen(strMedia) < cchDest )
        _tcscat_s( strDest, lstrlen(strMedia),strMedia );
    else
        return E_INVALIDARG;
...
HRESULT DXUtil_FindMediaFileCch( TCHAR* strDestPath, int cchDest, TCHAR* strFilename )
{
    ...
    if( INVALID_HANDLE_VALUE != file )
    {
        _tcsncpy_s( strDestPath, lstrlen(strDestPath)  ,strShortName, cchDest);
    ...
    _tcsncpy_s( strDestPath, lstrlen(strDestPath),strFilename, cchDest);
    strDestPath[cchDest-1] = 0; // _tcsncpy doesn't NULL term if it runs out of space
                                    ...
                                    DWORD cchDest, TCHAR* strDefault )
{
    ...
    if( ERROR_SUCCESS != RegQueryValueEx( hKey, strRegName, 0, &dwType, 
                                          (BYTE*)strDest, &cbDest ) )
    {
        _tcsncpy_s( strDest, lstrlen(strDest), strDefault, cchDest);
        strDest[cchDest-1] = 0;
...
#else
    strncpy_s( strDestination, lstrlen(strDestination),tstrSource, cchDestChar );
    strDestination[cchDestChar-1] = '\0';
    return S_OK;
#endif   
}
...
#else
    strncpy_s( tstrDestination,lstrlen(tstrDestination), strSource, cchDestChar );
...
VOID DXUtil_LaunchReadme( HWND hWnd, TCHAR* strLoc )
{
    ...
    if( strLastSlash )
    {
        _tcsncpy_s( strExeName,lstrlen(strExeName), &strLastSlash[1], MAX_PATH );
...
{
    int nResult = _sntprintf_s( strDest, lstrlen(strDest),cchDestChar, TEXT("{%0.8X-%0.4X-%0.4X-%0.2X%0.2X-%0.2X%0.2X%0.2X%0.2X%0.2X%0.2X}"),
